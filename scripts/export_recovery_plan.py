...
def main() -> None:
    parser = argparse.ArgumentParser(description="Export a markdown recovery plan for a QE run or staged workflow.")
    parser.add_argument("path", nargs="?", default=".")
    parser.add_argument("--output")
    args = parser.parse_args()

    source = Path(args.path).expanduser().resolve()
    records = [build_recommendation(parse_qe_dir(directory)) for directory in discover_dirs(source)]
